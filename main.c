...
void thread1 (void const *argument)
{
	...
	while(1)
	{
		// Test if vehicle count sensors are activated:
		if((GPIOA->IDR & VEHICLE_V) == VEHICLE_V)
		{
			vvc++;
		}
		...
		// Every 2 min resets vehicle count variables:
		if((TIM1->CNT) >= 65000)
		{
			hvc=0;
			vvc=0;
		...
		data	=	(hvc & (0x0000FFFF)) | ((vvc & (0x0000FFFF))<<16);
...
void thread3 (void const *argument)
{
	...
	while(1)
	{
		...
		if(event_message.status == osEventMessage)									// Check if message is recived properly.
		{
			...
			// Reciveing signals:
			event_signal = osSignalWait(0,(hor_delay/2));			// Wait for signal to apear, for half of the calculated delay.
			...
			HORoff_VERon();																		// Continue driveing traffic lights horizontal red and vertical green.
